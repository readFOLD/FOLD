...
updateStream = function(selector, modifier, options) {
...
addContextToStream =  function (streamShortId, contextBlock) { // TODO find a way to pick id safely to avoid potential collisions. Context block id uniqueness is currently not enforced.
  ...
  var deepstream = Deepstreams.findOne({shortId: streamShortId}, {fields: {'creationStep': 1, 'curatorId': 1}});
  ...
  pushObject[pushSelectorString] = _.pick(contextBlockToInsert, ['_id', 'type', 'source']); // TODO, anything else?;
  ...
  var numInserted = ContextBlocks.insert(contextBlockToInsert);

  if (!numInserted) {
    throw new Meteor.Error('Context block not inserted')
  }

  console.log(streamShortId);
