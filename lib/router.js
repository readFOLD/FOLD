...
var setMetaImage = function(imageUrl){
  if (Meteor.isClient){
    if (imageUrl){
      ...
      $('meta[property="og:image"]').attr('content', "https://fold.cm/FOLD_fb_image.png");
      $('meta[property="og:image:secure_url"]').attr('content', "https://fold.cm/FOLD_fb_image.png");
      $('meta[name="twitter:image"]').attr('content', "https://fold.cm/FOLD_twitter_image.png");
    }
  }
};
...
Router.route("read", {
  ...
  waitOn: function() { // subscribe and wait for story if don't have it yet
    ...
    return [Meteor.subscribe('readStoryPub', this.params.userPathSegment, shortId)]; // NOTE: this subscription is stopped as soon as the story is found the way this is currently structured as of 4/30/15
  },
  action: function() {
    if (this.ready()) {
      return this.render();
    ...
    if (this.ready()){
      ...
      if (story) {
        ...
        Session.set("horizontalSectionsMap", _.map(_.pluck(story.verticalSections, "contextBlocks"), function (cBlockIds, i) {
          ...
          return {
            verticalIndex: i,
            activeHeartbeats: story.heartbeats ? story.heartbeats.active[id] : 0,
