...
var singleDeepstreamSubscriptions = function(params) {
  ...
  if (Meteor.isClient) { // if full deepstream is already loaded due to visiting homepage, don't wait for it
    var deepstream = Deepstreams.findOne({shortId: shortId}, {reactive: false, fields: {contextBlocks: 1}});
    if (deepstream && deepstream.contextBlocks) { // confirm that full "watch deepstream" is loaded
      return
    }
  }
